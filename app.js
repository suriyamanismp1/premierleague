...
{
  let d=[managerName,managerAge, currentTeam, trophiesWon];
  return d;
}
...
function filterByAwardxTimes(awardName, country) {
  ...
  var data=awardsData.filter(player =>return  player.country == country);
  return data;
}
//Progression 8 - Filter players that won atleast ______ awards, belong to ______ team and are younger than ____
function filterByNoOfAwardsxTeamxAge(noOfAwards, team, age) {
  var data=players.filter(player => (player.age && player.team==team && player.awards.length >=noOfAwards))
  return data;
}
...
function SortByNamexAwardxTimes(awardName, noOfTimes, country) {
  var dataOne =filteredByAwardxTimes(awardName,noOfTimes);
